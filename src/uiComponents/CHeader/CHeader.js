    ...
    <div className="c-header">
      <span className="inner-container">
        ...
        <div className={`mobile-menu ${ menu ? "menu-open" : ""}`}>
          <Link
           to="/"
           onClick={() => {setCurrentPath("/"); setMenu(false)}}

        >Post a free job</Link>
          <Link
            to="/employee-and-agencies"
            onClick={() => {setCurrentPath("/employee-and-agencies"); setMenu(false)}}
            className={getActiveClassForPath(
              currentPath,
              "/employee-and-agencies"
            )}
          ...
          <Link
            to="/job-seekers"
            onClick={() => {setCurrentPath("/job-seekers"); setMenu(false)}}
            className={getActiveClassForPath(currentPath, "/job-seekers")}
          >
            Job Seekers
          </Link>
          <Link
            to="/jobs"
            onClick={() => {setCurrentPath("/jobs"); setMenu(false)}}
            className={getActiveClassForPath(currentPath, "/jobs")}
          >
            Jobs
          </Link>
          ...
          <Link
            to="/pricing"
            onClick={() => {setCurrentPath("/pricing"); setMenu(false)}}
            className={getActiveClassForPath(currentPath, "/pricing")}
          >
            Pricing
          </Link>
          <Link 
          to="/login" 
          onClick={() => {setCurrentPath("/login"); setMenu(false)}}
          className="login">
            Login
          </Link>
        </div>
        <nav className="align-items-center">
          ...
          <button onClick={()=>setMenu(!menu)} className="toggle-button">
              {menu ? ( <VscChromeClose className="close-icon"  size="25px"/>) :(<VscMenu className="menu-icon" size="25px"/>)}
          </button>
        </nav>
      </span>
