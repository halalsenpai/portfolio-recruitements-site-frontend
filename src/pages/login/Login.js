      ...
      <div className="c-card-container login-container">
        ...
        <div className="second-container">
          <div className="user-type">
            <span className={`${getIsActive(userTypes.SEEKER)}`} onClick={() => setUserType(userTypes.SEEKER)}>
              Job Seeker
            </span>
            <span className={`${getIsActive(userTypes.EMPLOYER)}`} onClick={() => setUserType(userTypes.EMPLOYER)}>
              Employer
            </span>
            <span className={`${getIsActive(userTypes.AGENCY)}`} onClick={() => setUserType(userTypes.AGENCY)}>
              Agency
            </span>
          </div>
          <Form className="c-form" onFinish={onFinish}>
            ...
            <Form.Item name="password" rules={Rules.passwordRule} className="c-password-input mb-2">
